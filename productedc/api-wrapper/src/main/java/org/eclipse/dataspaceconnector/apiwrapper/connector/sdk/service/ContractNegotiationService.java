...
public class ContractNegotiationService {
    ...
    public String initiateNegotiation(NegotiationInitiateRequestDto contractOfferRequest, String connectorControlPlaneBaseUrl, Map<String, String> headers, long validity) {
        var url = connectorControlPlaneBaseUrl + NEGOTIATION_PATH;
        var requestBody = RequestBody.create(
                injectValidityPeriod(typeManager.writeValueAsString(contractOfferRequest), validity),
                Utility.JSON
        );
    ...
    private String injectValidityPeriod(String origString, long validity){
        String retString = origString.replaceFirst("\"policy\"", "\"validity\":" + validity + ",\n\"policy\"");
        monitor.debug("##### Injected Body String: " + retString);
        return retString;
    }
